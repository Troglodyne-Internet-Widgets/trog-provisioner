...
subtest 'what it will not take' => sub {
    like( exception { recipe()->validate( vim_plugins => { x => { repo => 'o/r', ref => 'master' } } ) }, qr/ref/, 'a branch, which the cache would keep a year of' );
...
subtest 'the fragment fetches each tarball through the cache, into an emptied directory' => sub {
    my $out   = rendered();
    ...
    like( $async[0], qr{^/root/bin/fetch 'https://codeload\.github\.com/prabirshrestha/async\.vim/tar\.gz/$ref' 'perllsp\.async\.tar\.gz'$}, 'fetched as the pinned commit, through scripts/fetch' );
