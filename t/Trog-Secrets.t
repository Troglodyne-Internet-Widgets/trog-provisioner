...
subtest 'replace() sets what it names and leaves the rest of the store alone' => sub {
    ...
    # The whole reason this exists rather than write().  write() builds a fresh
    # database out of what it is handed, so against a real store it would leave
    # it holding one entry and nothing else.
    ...
    my %fresh = Trog::Secrets->read( $file, $pass, k => 'secret:guests/new.test/password' );
