...
* A guard in `enrich()` still earns its place where being handed the recipe and being on the right guest are different questions.  `grafanasyslog` reads a stream the collector on *this machine* receives, so it refuses a domain with no `logcollector` in `$opts{modules}` rather than producing a dashboard with nothing behind it.  That is a statement about where the recipe belongs, not a check that the depsolver failed to add something.
    - Example: `die "The grafanasyslog recipe draws what logcollector receives, and $opts{domain} has no logcollector to receive anything.\n" unless List::Util::any { $_ eq 'logcollector' } @{$opts{modules}};`
    - Not for a dependency you named yourself in `required_recipes`.  That one is already there, and a guard against it is a guard that cannot fire.
